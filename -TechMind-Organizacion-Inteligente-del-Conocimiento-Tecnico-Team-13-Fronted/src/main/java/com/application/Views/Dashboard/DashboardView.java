...
public class DashboardView extends VerticalLayout {
    ...
    private final com.application.service.ContenidoService contenidoService;
    private final com.application.service.UserSession userSession;

    public DashboardView(com.application.service.ContenidoService contenidoService, com.application.service.UserSession userSession) {
    ...
    private HorizontalLayout createKpiRow() {
        ...
        java.util.List<com.application.model.Contenido> contents = java.util.Collections.emptyList();
        if (userSession != null && userSession.getAuthenticatedUser() != null) {
            try {
                contents = contenidoService.listarPorUsuario(userSession.getAuthenticatedUser().getId());
            } catch (Exception e) {
        ...
        if (contents != null) {
            for (com.application.model.Contenido c : contents) {
                if (c.getTitulo() != null) {
    ...
    private HorizontalLayout createMiddleRow() {
        ...
        java.util.List<com.application.model.Contenido> contents = java.util.Collections.emptyList();
        if (userSession != null && userSession.getAuthenticatedUser() != null) {
            try { contents = contenidoService.listarPorUsuario(userSession.getAuthenticatedUser().getId()); } catch (Exception e) { System.err.println("[Dashboard] tags build error: " + e.getMessage()); }
        }
        ...
        java.util.List<com.application.model.Contenido> contentsAct = java.util.Collections.emptyList();
        if (userSession != null && userSession.getAuthenticatedUser() != null) {
            try { contentsAct = contenidoService.listarPorUsuario(userSession.getAuthenticatedUser().getId()); } catch (Exception e) { System.err.println("[Dashboard] activity fetch error: " + e.getMessage()); }
        }

        int added = 0;
        if (contentsAct != null && !contentsAct.isEmpty()) {
            for (com.application.model.Contenido c : contentsAct) {
                if (added >= 6) break;
                ...
                String time = c.getFechaCreacion() != null ? c.getFechaCreacion().toString() : "hace poco";
    ...
    private HorizontalLayout createBottomRow() {
        ...
        com.application.model.Contenido latest = null;
        if (userSession != null && userSession.getAuthenticatedUser() != null) {
            try {
                java.util.List<com.application.model.Contenido> cs = contenidoService.listarPorUsuario(userSession.getAuthenticatedUser().getId());
        ...
        Paragraph lpText = new Paragraph(latest != null && latest.getTexto() != null ? (latest.getTexto().length() > 220 ? latest.getTexto().substring(0, 220) + "..." : latest.getTexto()) : "No hay descripción disponible.");
