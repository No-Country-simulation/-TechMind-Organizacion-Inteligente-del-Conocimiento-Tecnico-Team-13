...
public class AnnotationsView extends HorizontalLayout {
    ...
    private java.util.List<Contenido> userContents = new java.util.ArrayList<>();
    private Contenido selectedContent = null;
    // Temporary in-memory notes map (not persisted) keyed by content ID; null key used for unsaved/general notes
    private java.util.Map<Long, String> tempNotes = new java.util.HashMap<>();
        ...
        if (userContents.isEmpty()) {
            ...
            for (Contenido c : userContents) {
                ...
                if (c.getTexto() != null && !c.getTexto().isBlank()) {
                    String[] sents = c.getTexto().split("(?<=[.!?])\\s+");
    ...
    private Component createWorkflowListItem(Contenido content, int progress, String category, int steps, boolean isActive) {
    ...
    private Component buildWorkflowDetailView() {
        ...
        String descriptionText = selectedContent.getTexto() != null && !selectedContent.getTexto().isBlank() ? excerpt(selectedContent.getTexto(), 400) : "No hay descripción disponible.";
        String category = selectedContent.getTipoContenido() != null ? selectedContent.getTipoContenido() : "General";
        String dateText = selectedContent.getFechaCreacion() != null ? selectedContent.getFechaCreacion().toString() : "Fecha desconocida";
    ...
    private Component createNotesEditor() {
        ...
        save.addClickListener(ev -> {
            Long key = selectedContent != null ? selectedContent.getId() : null;
    ...
    private Component buildWorkflowStepsFromContent(Contenido content) {
        VerticalLayout layout = new VerticalLayout();
        layout.setSpacing(true);
        layout.add(new H3("PASOS DEL WORKFLOW"));

        String text = content != null && content.getTexto() != null ? content.getTexto() : "";
    ...
    private void loadUserContents() {
        try {
            ...
            if (maybeUser.isPresent()) {
                userContents = contenidoService.listarPorUsuario(maybeUser.get().getId());
            } else {
    ...
    private int computeProgressForContent(Contenido content) {
        ...
        String text = content.getTexto() != null ? content.getTexto() : "";
