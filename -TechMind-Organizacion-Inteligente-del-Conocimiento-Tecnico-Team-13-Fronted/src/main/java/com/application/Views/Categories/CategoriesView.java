...
public class CategoriesView extends VerticalLayout implements BeforeEnterObserver {
    ...
    private final Map<String, List<Content>> categorizedContents = new LinkedHashMap<>();
    ...
    private void renderCategoriesGrid() {
        ...
        List<Map.Entry<String, List<Content>>> entries = categorizedContents.entrySet().stream()
                .filter(e -> {
        ...
        for (Map.Entry<String, List<Content>> entry : entries) {
    ...
    // =========================================================================
    private void openViewResourcesDialog(String categoryName, List<Content> contents) {
        Dialog dialog = new Dialog();
        ...
        Runnable renderResourceCards = () -> {
            ...
            List<Content> filtered = contents.stream()
                    .filter(c -> q.isBlank() || (c.getTitulo() != null && c.getTitulo().toLowerCase().contains(q))
                            || (c.getTextoPlano() != null && c.getTextoPlano().toLowerCase().contains(q)))
                    .toList();
    ...
    private Component createResourceItemCard(Content content, String currentCategory, Dialog parentDialog) {
        ...
        Span timeSpan = new Span(formatTimeAgo(content.getCreatedAt()));
        ...
        if (content.getTextoPlano() != null && !content.getTextoPlano().isBlank()) {
            description = excerpt(content.getTextoPlano(), 180);
    ...
    private void openContentDetailModal(Content content) {
        Dialog dialog = new Dialog();
        ...
        Span categorySpan = new Span("Categoría: " + (content.getTipoContenido() != null ? content.getTipoContenido() : "General"));
        ...
        contentArea.setValue(content.getTextoPlano() != null && !content.getTextoPlano().isBlank()
    ...
    private void openQuickMoveDialog(Content content, Dialog parentDialog) {
        ...
        List<String> targetCategories = activeCategories.stream()
                .filter(c -> !c.equalsIgnoreCase(content.getTipoContenido()))
                .toList();
        ...
        Button confirm = new Button("Mover", e -> {
            ...
            try {
                supabaseService.updateContentCategory(content.getId(), dest);
    ...
    private void confirmDeleteSingleContent(Content content, Dialog parentDialog) {
    ...
    // =========================================================================
    private void openEditCategoryDialog(String categoryName, List<Content> contents) {
        Dialog dialog = new Dialog();
        ...
        Button renameBtn = new Button("Guardar Nombre", e -> {
            ...
            try {
                Optional<User> maybeUser = Optional.ofNullable(userSession.getAuthenticatedUser());
                if (maybeUser.isPresent()) {
                    supabaseService.renameCategoryForUser(maybeUser.get().getId(), categoryName, newName);
                }
        ...
        if (contents.isEmpty()) {
            ...
            Set<UUID> selectedContentIds = new HashSet<>();
            ...
            for (Content c : contents) {
                ...
                singleTargetCombo.addValueChangeListener(ev -> {
                    if (ev.getValue() != null && !ev.getValue().isBlank()) {
                        try {
                            supabaseService.updateContentCategory(c.getId(), ev.getValue());
            ...
            moveSelectedBtn.addClickListener(e -> {
                ...
                try {
                    supabaseService.moveContentsToCategory(new ArrayList<>(selectedContentIds), targetCat);
    ...
    // =========================================================================
    private void openDeleteCategoryDialog(String categoryName, List<Content> contents) {
        Dialog dialog = new Dialog();
            ...
            Button confirmBtn = new Button("Confirmar", e -> {
                ...
                if (moveFirst) {
                    ...
                    try {
                        ...
                        supabaseService.moveContentsToCategory(ids, dest);
                        activeCategories.remove(categoryName);
                    ...
                    try {
                        for (Content c : contents) {
                            supabaseService.deleteContentForUser(c.getId(), c.getStoragePath());
                        }
                        activeCategories.remove(categoryName);
    ...
    private void renderSummaryCard() {
        ...
        long unassigned = allContents.stream()
                .filter(c -> c.getTipoContenido() == null || c.getTipoContenido().isBlank() || c.getTipoContenido().equalsIgnoreCase("General"))
                .count();
